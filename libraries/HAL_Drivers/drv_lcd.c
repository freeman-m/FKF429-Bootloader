...
rt_uint16_t ltdc_layer0[LTDC_HEIGHT][LTDC_WIDTH] __attribute__((at(SDRAM_LCD_BUF1)));
rt_uint16_t ltdc_layer1[LTDC_WIDTH][LTDC_HEIGHT] __attribute__((at(SDRAM_LCD_BUF2)));


static void set_pixel(const char *pixel, int x, int y)
{
    *(rt_uint16_t*)((rt_uint32_t)ltdc_layer0[0] + 2 * (LCD_WIDTH * y + x)) = (*(rt_uint16_t*)pixel);
}

static void get_pixel(char *pixel, int x, int y)
{
	*(rt_uint16_t*)pixel = *(rt_uint16_t*)((rt_uint32_t)ltdc_layer0[0] + 2 * (LCD_WIDTH * y + x));
}
...
void draw_hline(const char *pixel, int x1, int x2, int y)
{
	while (x1 <= x2)
	{
		*(rt_uint16_t*)((rt_uint32_t)ltdc_layer0[0] + 2 * (LCD_WIDTH * y + x1)) = (*(rt_uint16_t*)pixel);	
		x1++;
	}
}

struct rt_device_graphic_ops rgblcd_ops =
    {
        set_pixel,
        get_pixel,
        draw_hline,
...
static void lcd_thread(void *param)
{
    ...
    while(1)
    {
        ...
        {
            lcd->lcd_info.framebuffer[2 * i] = *(__IO uint8_t *)(0xD0800000 + 2 * i);//0x00;
            lcd->lcd_info.framebuffer[2 * i + 1] =*(__IO uint8_t *)(0xD0800000 + 2 * i + 1);//= 0xF8;
        }
		#endif
...
static rt_err_t drv_lcd_init(struct rt_device *device)
{
	...
	rt_thread_t tid;
    tid = rt_thread_create("lcd_refresh", lcd_thread, RT_NULL, 2048, RT_THREAD_PRIORITY_MAX - 3, 10);
    if (tid != RT_NULL)
...
static rt_err_t drv_lcd_control(struct rt_device *device, int cmd, void *args)
{
    ...
    case RTGRAPHIC_CTRL_RECT_UPDATE:
    {
        ...
        if (_lcd.cur_buf)
        {
            ...
            memcpy(_lcd.front_buf, ltdc_layer0/*_lcd.lcd_info.framebuffer*/, LCD_BUF_SIZE);
        ...
        else
        {
            ...
            memcpy(_lcd.back_buf, ltdc_layer0/*_lcd.lcd_info.framebuffer*/, LCD_BUF_SIZE);
            /* Configure the color frame buffer start address */
            LTDC_LAYER(&LtdcHandle, 0)->CFBAR &= ~(LTDC_LxCFBAR_CFBADD);
...
int lcd_test()
{
    ...
    while (cnt)
    {
        ...
        for (int i = 0; i < LCD_BUF_SIZE / 2; i++)
        {
            //lcd->lcd_info.framebuffer[2 * i] = *(__IO uint8_t *)(SDRAM_LCD_BUF1 + 2 * i);//0x00;
            //lcd->lcd_info.framebuffer[2 * i + 1] =*(__IO uint8_t *)(SDRAM_LCD_BUF1 + 2 * i + 1);//= 0xF8;
        }
        lcd->parent.control(&lcd->parent, RTGRAPHIC_CTRL_RECT_UPDATE, RT_NULL);
        rt_thread_mdelay(1000);
        /* green */
